...
void correctYoloBoxes(std::vector<detection> &dets, int det_num, int image_h,
                      int image_w, int input_height, int input_width) {
  // 计算缩放比例和padding，与Python代码保持一致
  float scale =
      std::min((float)input_width / image_w, (float)input_height / image_h);
  ...
  for (int i = 0; i < det_num; ++i) {
    ...
    printf("Det[%d]: input_bbox(%.1f,%.1f,%.1f,%.1f) -> "
           ...
           i, cx, cy, w, h, dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w,
           dets[i].bbox.h);
#endif
  }
}
...
int getDetections(CVI_TENSOR *output, int32_t input_height, int32_t input_width,
                  int classes_num, CVI_SHAPE output_shape, float conf_thresh,
  ...
  int channels =
      output_shape.dim[1]; // 应该是4(bbox) + 1(objectness) + classes_num
...
// 根据球的位置控制小车移动
void controlMotor(Motor &motor, float ball_x, float ball_y, int image_width,
                  int image_height) {
  // 计算球相对于图像中心的位置
...
static int vi_get_frame_as_bgr(CVI_U8 chn, cv::Mat &bgr_image) {
  ...
  if (CVI_VI_GetChnFrame(0, chn, &stVideoFrame, 3000) == 0) {
    ...
    CVI_SYS_IonInvalidateCache(stVideoFrame.stVFrame.u64PhyAddr[0], vir_addr,
                               image_size);
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < output_num; i++) {
    ...
    printf("Output[%d] shape: [%d, %d, %d, %d]\n", i, output_shape[i].dim[0],
           output_shape[i].dim[1], output_shape[i].dim[2],
           output_shape[i].dim[3]);
  }
